...
def transform_podcast_transcript(transcript, parent_id):
    ...
    for chunk in chunks:
        ...
        summary = openai.complete(summary_prompt, source="PODCAST_TOOLKIT", parent_id=parent_id,
                                  stop_tokens=['The summary:'])
        key_points = openai.complete(key_points_prompt, source="PODCAST_TOOLKIT", parent_id=parent_id,
                                     stop_tokens=['The key points:'])
        links_to_include = openai.complete(links_to_include_prompt, source="PODCAST_TOOLKIT", parent_id=parent_id,
                                           stop_tokens=['The links to include:'])
        summary_chunks.append(summary)
        key_points_chunks.append(key_points)
        links_to_include_chunks.append(links_to_include)

    summary = openai.complete(summarize_podcast_section_summaries(summary_chunks), source="PODCAST_TOOLKIT",
                              parent_id=parent_id, )
    key_points = openai.complete(summarize_podcast_section_key_points(key_points_chunks), source="PODCAST_TOOLKIT",
                                 parent_id=parent_id, )
    links_to_include = openai.complete(summarize_podcast_section_links_to_include(links_to_include_chunks),
                                       source="PODCAST_TOOLKIT", parent_id=parent_id, )
    show_notes = openai.complete(generate_show_notes(summary, key_points, links_to_include), source="PODCAST_TOOLKIT",
                                 parent_id=parent_id, )

    return summary, key_points, links_to_include, show_notes


def create_embeddings_for_podcast_transcript(transcript, parent_id):
    ...
    for chunk in chunks:
        embeddings.append(openai.embeddings(chunk, source="PODCAST_TOOLKIT", parent_id=parent_id), )
